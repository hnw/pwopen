...
const isValidEpochMs = (value: number): boolean =>
...
const recordTimingAttributes = (span: Span, timing: RequestTiming): void => {
	...
	if (canSplitTls) {
		const tcpHandshakeMs = durationMs(
			timing.connectStart,
		...
		const tlsNegotiationMs = durationMs(
			timing.secureConnectionStart,
			timing.connectEnd,
		);
...
const startHopSpan = (
	...
	const hopStartTime = isValidEpochMs(timing.startTime)
		? timing.startTime
		: undefined;
	return hopStartTime
...
const recordRedirectSpans = async (
	tracer: Tracer,
	chain: Request[],
): Promise<void> => {
...
async function navigateWithRetries(
	...
	for (let attempt = 0; attempt <= maxAttempts; attempt += 1) {
		...
		try {
				...
				try {
					...
					if (response) {
						try {
							...
							console.warn(
								`[WARN] Failed to record navigation timing: ${String(error)}`,
							);
						}
					}
				} catch (error) {
					...
					if (lastNavigationRequest) {
						try {
							await recordNavigationTimingFromRequest(
								span,
								lastNavigationRequest,
								tracer,
							);
						} catch (timingError) {
							console.warn(
			...
			console.warn(
				`[WARN] Navigation failed, retrying (${attempt + 1}/${maxAttempts}): ${url}`,
			);
		} finally {
...
async function waitForRenderSettled(page: Page, tracer: Tracer): Promise<void> {
	...
	let sawNavigation = false;
	try {
		sawNavigation = await withSpan(
			tracer,
			"render.wait.main_frame_idle",
			async (span) => {
	...
	try {
		await withSpan(tracer, "render.wait.fonts_raf", async () => {
			await page.evaluate(async () => {
				const fontsReady = (
					document as { fonts?: { ready?: Promise<unknown> } }
				).fonts?.ready;
				if (fontsReady) {
					await fontsReady;
				}
			...
			try {
				await withSpan(tracer, "page.process", async (span) => {
					...
					if (options.screenshot) {
						const screenshot = await withSpan(
							tracer,
							"page.screenshot",
							async (shotSpan) => {
